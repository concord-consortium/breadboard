...
The Rack server also proxies database requests for the full activity set available on the couchdb server running
here: http://couchdb.cosmos.concord.org/sparks/
...
Here are some additional instructions for configuring Apache to acomplish the same goals:
