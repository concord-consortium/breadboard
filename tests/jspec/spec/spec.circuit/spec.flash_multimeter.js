describe 'Using multimeter with mock Flash connection'
    ...
    it "should send an RMS current to flash when in current mode and both probes added"

      // we add a 100 ohm resistor
      breadModel('insertComponent', 'resistor', {"connections": 'a1,a2', "colors": 'brown,black,brown,gold'});
      breadModel('insertComponent', 'function generator', {"UID": "source", "connections": 'b1,b3', "amplitude": 10, "frequencies": [1000]});
      ...
      sparks.flash.sendCommand = function(command, value) {
        command.should.be "set_multimeter_display"
        value.should.be "  7 0.7"
        sendCalled = true;
      }

      receiveEvent('connect', 'probe|probe_red|b3', 0);     // will call sendCommand
      sendCalled.should.be true
